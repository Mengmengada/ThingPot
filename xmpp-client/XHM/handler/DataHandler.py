...
def datahandler_handle_event_req(self, iq):
    """
           ...
           """
    print "____________________datahandler_handle_event_req is called_____________________"
    seqnr = iq['req']['seqnr'];
    error_msg = '';
    req_ok = True;
    ...
    if len(iq['req']['nodes']) > 0:
        for n in iq['req']['nodes']:
            if not n['nodeId'] in self.nodes:
                # req_ok = False;
                error_msg = "Inva-lid nodeId " + n['nodeId'];
    ...
    if req_ok:
        ...
        # print("added session: " + str(self.sessions))
        ReplyManager.form_iq_accept(self,iq,request_delay_sec,seqnr)
        # iq.reply();
        ...
        if self.threaded:
            ...
            tr_req = Thread(target=self._threaded_node_request,
                            args=(session, process_fields, req_flags))  # call --Meng
            tr_req.start()
            print("started thread")
        else:
